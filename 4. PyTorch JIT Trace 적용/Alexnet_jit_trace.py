...
# Evaluate the model on JIT Trace Mode
correct = 0
total = 0
test_model = AlexNet().to(device)
test_model.load_state_dict(torch.load("best_model.pth"))
...
with torch.no_grad():
    for data in test_loader:
        ...
        # JIT Trace
        traced_test_model = torch.jit.trace(test_model, images)

        # optimized
        optimized_traced_model = torch.jit.optimize_for_inference(traced_test_model)

        start.record()
        # outputs = traced_test_model(images)
        outputs = optimized_traced_model(images)
        end.record()
